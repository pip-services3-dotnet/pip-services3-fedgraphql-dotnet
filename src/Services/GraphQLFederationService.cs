...
using PipServices3.GraphQL.Federation.Common;
...
namespace PipServices3.GraphQL.Federation.Services
{
	...
	/// </code>
	/// </example>
	public abstract class GraphQLFederationService : IOpenable, IConfigurable, IReferenceable, IUnreferenceable, IInitializable
    {
		...
		public GraphQLFederationService(string schemaFile = null)
        {
        ...
        public virtual void SetReferences(IReferences references)
        {
			...
			if (_debug) Console.WriteLine($"GraphQLFederationService. Endpoint initialized");
		}
		...
		private HttpEndpoint CreateLocalEndpoint()
		{
			...
			if (_debug) Console.WriteLine("GraphQLFederationService. Created local endpoint");
        ...
        public async virtual Task OpenAsync(string correlationId)
        {
			...
			if (_localEndpoint)
			{
				await _endpoint.OpenAsync(correlationId).ContinueWith(task =>
				{
					...
					if (_debug) Console.WriteLine("GraphQLFederationService. Opened");
				});
			}
			else
			{
        ...
        public virtual Task CloseAsync(string correlationId)
        {
            if (IsOpen())
            {
				...
				if (_debug) Console.WriteLine("GraphQLFederationService. Closed");
		...
		public virtual void ConfigureServices(IServiceCollection services)
		{
			...
			if (_debug) Console.WriteLine("GraphQLFederationService. Configured services");
		}
		...
		public virtual void ConfigureApplication(IApplicationBuilder applicationBuilder)
		{
			...
			if (_debug) Console.WriteLine("GraphQLFederationService. Configured application");
		}
		...
		protected void RegisterSchema(IRequestExecutorBuilder builder)
		{
			if (!string.IsNullOrWhiteSpace(_schemaFile))
			{
				...
				if (_debug) Console.WriteLine($"GraphQLFederationService. Schema file registered (fileName = {_schemaFile}, content.Length = {schemaContent.Length})");
				return;
			}

			if (_debug) Console.WriteLine("GraphQLFederationService. Schema file not registered");
		}
		...
		protected object RegisterController(IRequestExecutorBuilder builder)
		{
			...
			if (_debug) Console.WriteLine($"GraphQLFederationService. Controller registered ({controller.GetType().FullName})");
		...
		protected void RegisterInterceptor(IRequestExecutorBuilder builder)
		{
			...
			if (nextAction != null)
			{
				...
				if (_debug) Console.WriteLine("GraphQLFederationService. Interceptor registered");
			}
		}
